...
class Species:
    def __init__(self, params):
        ...
        self.species_birth_time_lambda = params['species_birth_time_lambda']
        self.species_death_time_lambda = params['species_death_time_lambda']
    ...
    def birth_time(self, agent, birth_time_decide=None):
        if birth_time_decide is not None:
            agent.birth_time = birth_time_decide
        else:
            agent.birth_time = np.random.exponential(1 / self.species_birth_time_lambda)
        return agent.birth_time

    def death_time(self, agent, death_time_decide=None):
        if death_time_decide is not None:
            agent.death_time = death_time_decide
        else:
            agent.death_time = np.random.exponential(1 / self.species_death_time_lambda)
            # check if birth time is larger than death time
            while agent.birth_time > agent.death_time:
                agent.death_time = np.random.exponential(1 / self.species_death_time_lambda)
...
class Prey(Species):
    def __init__(self, ID, x, y, params, status=None, birth_time_decide=None, death_time_decide=None):
        super().__init__(params)
        self.ID = ID
        self.birth_time = self.birth_time(self, birth_time_decide)
        self.death_time = self.death_time(self, death_time_decide)
        ...
        if not self.being_caught:
            for predator in predators:
                ...
                if distance <= (self.activity_radius + predator.activity_radius):
                    ...
                    time_for_predation = np.random.exponential(distance)
    ...
    for agent in population:
        while agent.num_offspring > 0:
            if np.random.rand() < 1 - current_pop_size / pop_limit:
                ...
                birth_time_decide = current_time + np.random.exponential(1 / agent.species_birth_time_lambda)
                death_time_decide = current_time + np.random.exponential(1 / agent.species_death_time_lambda)
                if agent.species_name == 'prey':
                    new_offspring.append(
                        Prey(ID, x, y, prey_params, None, birth_time_decide, death_time_decide))
                if agent.species_name == 'predator':
                    new_offspring.append(
                        Predator(ID, x, y, predator_params, None, birth_time_decide, death_time_decide))
                current_pop_size += 1
            else:
                break
...
def update_agent_status(population, current_time):
    for agent in population:
...
prey_params = {
    ...
    'y_grid': y_grid,
    'species_name': 'prey',
    'species_birth_time_lambda': 0.15,
    'species_death_time_lambda': 0.08,
    'species_offspring_possion_mean': 3,
    'init_species_size': 500,
    'activity_radius_max': 2,
    'speed_max': 2
}
predator_pop_limit = 900
predator_params = {
    'x_grid': x_grid,
    'y_grid': y_grid,
    'species_name': 'predator',
    'species_birth_time_lambda': 0.15,
    'species_death_time_lambda': 0.11,
    'species_offspring_possion_mean': 1,
    'init_species_size': 50,
    'activity_radius_max': 10,
    'speed_max': 5
}

# Create prey and predator populations
preys = [Prey(i, None, None, prey_params, "active", 0, None) for i in range(prey_params['init_species_size'])]
predators = [Predator(i, None, None, predator_params, 'active', 0, None) for i in
...
if __name__ == '__main__':
    ...
    for current_time in tqdm(range(1, time + 1)):
        ...
        update_agent_status(preys, current_time)
        update_agent_status(predators, current_time)
